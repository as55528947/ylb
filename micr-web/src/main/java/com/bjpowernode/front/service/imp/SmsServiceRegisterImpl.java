...
public class SmsServiceRegisterImpl implements SmsService {
    @Resource
    ...
    public boolean sendSms(String phone) {
        ...
        //更新配置文件中短信内容 %s 【利盈宝】你的验证码是：%s，3分钟内有效！请勿泄露他人
        String content = String.format(jdwxSmsConfig.getContent(), random);
            ...
            if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK){
                ...
                if (StringUtils.isNotBlank(text)){
                    ...
                    if ("Success".equalsIgnoreCase(jsonObject.getJSONObject("result").getString("ReturnStatus"))){
                        smsSendResult = true;
                        //把短信验证码存到redis中
                        stringRedisTemplate.boundValueOps(RedisKey.KEY_SMS_CODE_REG + phone).set(random, 3, TimeUnit.MINUTES);
                    }
                }
            }
        }catch (Exception e){
